...
- https://developer.mozilla.org/ru/docs/Web/API/Fetch_API/Using_Fetch - MDN
  :::

::: danger

**`fetch(url[, options])`** - интерфейс для получения ресурсов (в том числе по сети)
...
getResource("https://jsonplaceholder.typicode.com/todos/1").then((body) =>
  console.log(body),
);
```
...
::: details Передача одного значения
