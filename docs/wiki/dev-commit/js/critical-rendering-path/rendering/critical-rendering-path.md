...
- HTML может быть исполнен по частям. _Документ не должен быть загружен полностью_ для того, чтобы контент начал появляться на странице
- _DOM-дерево помещается в JavaScript-объект Document_
...
- CSS считается _«блокирующим обработку ресурсом»_. Это значит, что Render-дерево не может быть построено без полного первоначального разбора CSS
...
- _CSSOM перестраивается полностью каждый раз_
...
<v-breadcrumbs :items="['Chrome Dev Tools', 'Performance', 'Event Log']" />
<v-breadcrumbs :items="['Chrome Dev Tools', 'Network', 'Record network activity']" />
