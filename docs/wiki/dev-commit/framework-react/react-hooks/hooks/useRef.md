...
:::
Похож на useState и также создает состояние. Состояние, определенное через useRef сохраняется между ренедерами компонента, но при этом при изменении значения useRef не происхожит перерендера
