...
- Сначала определяются _оптимистичная (O = Optimistic)_, _пессимистичная (P = Pessimistic)_ и _реалистичная\средняя (M = Middle)_ оценки
...
- Далее полученные значения P, M и O подставляются в формулу: _(O + 4 M + P) / 6_
