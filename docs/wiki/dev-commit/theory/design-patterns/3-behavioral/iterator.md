...
- _Задача_: перебрать коллекцию. Коллекция - совокупность объектов, объединенных по определенным критериям
- _Идея_: вынести алгоритм обхода коллекции из самой коллекции в отдельный класс, который содержит методы `next` и `hasNext`
  :::
...
<v-two :title="['ArrayIterator', 'ObjectIterator']">
  <template #first>
